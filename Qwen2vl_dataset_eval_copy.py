...
Missing: none OR gloves, goggles, hardhat, mask, safety vest
...
def parse_model_output(text: str) -> dict:
    ...
    comp_match = re.search(r"compliance\s*[:\-]\s*(compliant|non-compliant)", text, re.I)
    miss_match = re.search(r"missing\s*[:\-]\s*(.+)", text, re.I)

    if comp_match:
        compliance = comp_match.group(1).lower()

    if miss_match:
        miss_text = miss_match.group(1).strip().lower()
        if miss_text != "none":
            for item in ["gloves", "goggles", "hardhat", "mask", "safety vest"]:
                if item in miss_text:
